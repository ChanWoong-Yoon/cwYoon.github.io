...
const RESEARCH = [
  ...
  },
  {
    ...
    body: [
      "Optogenetic reactivation of a hippocampal ensemble is sufficient to drive recall, and enforcing engram-level connectivity can produce a memory for something that never happened. Both depend on tagging: the neurons must be labeled during learning or be genetically accessible, which confines the method to representations the animal already has. Nor is allocation arbitrary — which neurons join an ensemble follows from excitability and existing connectivity — so driving a chosen set to arbitrary activity produces activity, not a representation.",
      "GIVER sidesteps that problem by not identifying anyone. Treat the activity of N neurons as a point in N-dimensional state space and a concept becomes a region rather than a list of cells, with related concepts sitting nearer than unrelated ones. The coordinate is also what makes the idea actionable, and it shifts what has to be selected. Which neurons are used does not matter; where their activity sits does. Given a target coordinate, the weights between a freely chosen population and the reference ensembles could be adjusted until that population’s activity settles at the intended point.",
